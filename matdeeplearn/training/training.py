...
from matdeeplearn.process import process, process_cl, process_nm, process_nm_ve, process_nm_as
...
#  Training functions
################################################################################
##Pretrain DSSL step, runs model in train mode
def pretrain_dssl(model, model_linear, optimizer, optimizer_linear, loader_cl1, loader_cl2, loader_nm, criterion_nm, criterion_cl, rank):
    ...
    for data_cl1, data_cl2, data_nm in zip(loader_cl1, loader_cl2, loader_nm):
        ...
        #ablation study
        #loss = loss_nm   ## masking
        #loss = loss_cl  ## contrastive learning
...
##Evaluation DSSL step, runs model in eval mode
def evaluate_pretrain_dssl(loader_cl1, loader_cl2, loader_nm, model, model_linear, criterion_nm, criterion_cl, rank, out=False):
    ...
    return val_error, val_acc_atom


##Pretrain DSSL+micro step, runs model in train mode
def pretrain_dssl_micro(model, linear_nm, linear_micro, optimizer, optimizer_nm, optimizer_micro, loader_cl1, loader_cl2, loader_nm, criterion_nm, criterion_micro, criterion_cl, rank, microp="AS"):
    ...
    for data_cl1, data_cl2, data_nm in zip(loader_cl1, loader_cl2, loader_nm):
        ...
        if microp == "VE":
            target_micro = data_nm.y1
            target_micro = torch.argmax(target_micro, 1) 
        else:
            target_micro = data_nm.y1.unsqueeze(1)
        
        node_pre1, node_cl1, node_cl2 = model(data_nm, data_cl1, data_cl2)
        
        pred_node = linear_nm(node_pre1[data_nm.atom_index])
        pred_micro = linear_micro(node_pre1[data_nm.atom_index])
        ...
        ## Microproperty prediction
        loss_micro = criterion_micro(pred_micro, target_micro)
        ...
        #loss = 0.33*loss_nm + 0.33*loss_micro + 0.33*loss_cl    # consider
        loss = 0.1*loss_nm + 0.1*loss_micro + loss_cl    # consider
        loss.backward()
...
##Evaluation DSSL+micro step, runs model in eval mode
def evaluate_pretrain_dssl_micro(loader_cl1, loader_cl2, loader_nm, model, linear_nm, linear_micro, criterion_nm, criterion_micro, criterion_cl, rank, out=False, microp="AS"):
    ...
    for data_cl1, data_cl2, data_nm in zip(loader_cl1, loader_cl2, loader_nm):
        ...
        with torch.no_grad():
            ...
            #target = torch.tensor(data_nm.y, dtype=torch.long)
            target = data_nm.y
            target = torch.argmax(target, 1) 
...
##Train FT step, runs model in train mode
...
##Evaluation FT step, runs model in eval mode
...
def trainer_dssl(
    ...
    epochs,
    verbosity,
    filename = "pretrain_dssl_tmp.pth",
):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_error, train_acc_atom = pretrain_dssl(model, model_linear, optimizer, optimizer_linear, train_loader_cl1, train_loader_cl2, train_loader_nm, loss_nm, loss_cl, rank=rank)
        ...
        val_error, val_acc_atom = evaluate_pretrain_dssl(val_loader_cl1, val_loader_cl2, val_loader_nm, model.module, model_linear, loss_nm, loss_cl, rank=rank, out=False)
        ...
        if epoch % int(verbosity) == 0:
            if rank in (0, "cpu", "cuda"):
                print(
                    "Epoch: {:04d}, Learning Rate: {:.6f}, Training Error: {:.5f}, Validation Error: {:.5f},Train Acc of Atom: {:.5f}, Val Acc of Atom: {:.5f}, Time per epoch (s): {:.5f}".format(
                        epoch, lr, train_error, val_error, train_acc_atom, val_acc_atom, epoch_time
                    )
                )
    ...
    return model_best, num_epoch, num_train_error, num_val_error, num_train_acc_node, num_val_acc_node

## DSSL+micro trainer
def trainer_dssl_micro(
    rank,
    world_size,
    model,
    linear_nm,
    ...
    epochs,
    verbosity,
    filename = "pretrain_dssl_micro_tmp.pth",
    microp = "AS",
):
    ...
    for epoch in range(1, epochs + 1):
        ...
        ##Train model
        train_error, train_acc_atom = pretrain_dssl_micro(model, linear_nm, linear_micro, optimizer, optimizer_nm, optimizer_micro, train_loader_cl1, train_loader_cl2, train_loader_nm, loss_nm, loss_micro, loss_cl, rank=rank, microp=microp)
        ...
        val_error, val_acc_atom = evaluate_pretrain_dssl_micro(val_loader_cl1, val_loader_cl2, val_loader_nm, model.module, linear_nm, linear_micro, loss_nm, loss_micro, loss_cl, rank=rank, out=False, microp=microp)
        ...
        if epoch % int(verbosity) == 0:
            if rank in (0, "cpu", "cuda"):
                print(
                    "Epoch: {:04d}, Learning Rate: {:.5f}, Training Error: {:.4f}, Validation Error: {:.4f},Train Acc of Atom: {:.4f}, Val Acc of Atom: {:.4f},  Time per epoch (s): {:.3f}".format(
                        epoch, lr, train_error, val_error, train_acc_atom, val_acc_atom, epoch_time
                    )
                )
...
def model_setup(
    ...
    if input_model_file != "":  
        print(f"Loading pretrained model: {input_model_file}")
...
def train_dssl(
    ...
    model, num_epoch, num_train_error, num_val_error, num_train_acc_node, num_val_acc_node = trainer_dssl(
        rank,
        world_size,
        model,
        model_linear,
    ...
    #loss_matri = np.vstack((num_epoch, num_train_error, num_val_error, num_train_acc_node, num_val_acc_node))
...
###pretraining with cl, nm, and as/ve
def train_dssl_micro(
    rank,
    world_size,
    data_path,
    job_parameters=None,
    ...
    if microp == "AS":    
        dataset_nm = process_nm_as.get_dataset(data_path, training_parameters["target_index"], False)
    else:
        dataset_nm = process_nm_ve.get_dataset(data_path, training_parameters["target_index"], False)
    ...
    if microp == "AS":
        linear_micro = torch.nn.Linear(115, 1).to(rank)
    else:
        linear_micro = torch.nn.Linear(115, 17).to(rank)
    ...
    optimizer_micro = torch.optim.Adam(linear_micro.parameters(), lr=0.0005, weight_decay=0)
    ...
    if microp == "AS":
        criterion_micro = torch.nn.L1Loss() ## regression loss
    else:
        criterion_micro = torch.nn.CrossEntropyLoss() ## classification loss

    criterion_cl = torch.nn.CosineEmbeddingLoss()
    
    ##Start training
    job_name = str(job_parameters["job_name"])
    model, num_epoch, num_train_error, num_val_error, num_train_acc_node, num_val_acc_node = trainer_dssl_micro(
        rank,
        world_size,
        model,
        linear_nm,
...
def train_cv_finetune(
    ...
    for index in range(0, len(cv_dataset)):
        ...
        model, _, _, _ = trainer_finetune(
            ...
            model_parameters["epochs"],
            training_parameters["verbosity"],
            str(job_parameters["job_name"]) + "_cv_tmp.pth",
        )
