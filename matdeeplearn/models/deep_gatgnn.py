    ...
    def calc(self, data):
        ...
        for i in range(0, len(self.pre_lin_list_N)):
            if i == 0:
                ...
                out_e = getattr(F, 'leaky_relu')(out_e,0.2)       
        prev_out_x = out_x         
        #print('prev_out_x: ', prev_out_x.shape)
        ...
        node_presentation = out_x    
        
        return node_presentation   
        
        
    def forward(self, data_nm, data_cl1, data_cl2):

        nr1 = self.calc(data_nm)
        nr2 = self.calc(data_cl1)
        nr3 = self.calc(data_cl2)
    
        return nr1, nr2, nr3


class FINETUNE_DEEP_GATGNN(torch.nn.Module):
    def __init__(self, data, dim1=64, dim2=64, pre_fc_count=1, gc_count=5, post_fc_count=1, pool="global_add_pool", pool_order="early", batch_norm="True", batch_track_stats="True", act="softplus", dropout_rate=0.0, **kwargs):
        super(FINETUNE_DEEP_GATGNN, self).__init__()
        ...
        self.deeper_gat = DEEP_GATGNN(data, dim1=dim1, dim2=dim2, pre_fc_count=pre_fc_count, gc_count=gc_count, batch_norm=batch_norm, batch_track_stats=batch_track_stats, act=act, dropout_rate=dropout_rate)
    ...
    def from_pretrained(self, model_file):        
        #self.deeper_gat.load_state_dict(torch.load(model_file)["model_state_dict"], strict=False)
        self.deeper_gat.load_state_dict(torch.load(model_file)["model_state_dict"])
    ...
    def forward(self, data):
        ...
        out_x = self.deeper_gat.calc(data)
