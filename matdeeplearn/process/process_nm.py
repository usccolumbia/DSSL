...
def get_dataset(data_path, target_index, reprocess="False", processing_args=None):
    ...
    if os.path.exists(os.path.join(data_path, processed_path, "data_nm.pt")) == True:
        dataset = StructureDataset(
            data_path,
            processed_path,
            transforms,
        )
    elif os.path.exists(os.path.join(data_path, processed_path, "data_nm0.pt")) == True:
...
class StructureDataset_large(Dataset):
    ...
    def processed_file_names(self):
        # file_names = ["data.pt"]
        file_names = []
        for file_name in glob.glob(self.processed_dir + "/data_nm*.pt"):
    ...
    def get(self, idx):
        data = torch.load(os.path.join(self.processed_dir, "data_nm{}.pt".format(idx)))
        return data
...
def process_data(data_path, processed_path, processing_args):
    ...
    if processing_args["dataset_type"] == "inmemory":
        ...
        torch.save((data, slices), os.path.join(data_path, processed_path, "data_nm.pt"))
        ...
        for i in range(0, len(data_list)):
            torch.save(
                data_list[i],
                os.path.join(
                    os.path.join(data_path, processed_path), "data_nm{}.pt".format(i)
                ),
            )
...
################################################################################
##data mask. One atom only mask once. Mask ratio 0.15 
