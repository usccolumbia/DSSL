...
def get_dataset(data_path, target_index, reprocess="False", processing_args=None):
    ...
    if os.path.exists(os.path.join(data_path, processed_path, "data_nmas.pt")) == True:
        dataset = StructureDataset(
            data_path,
            processed_path,
            transforms,
        )
    elif os.path.exists(os.path.join(data_path, processed_path, "data_nmas0.pt")) == True:
...
class StructureDataset_large(Dataset):
    ...
    def processed_file_names(self):
        # file_names = ["data.pt"]
        file_names = []
        for file_name in glob.glob(self.processed_dir + "/data_nmas*.pt"):
    ...
    def get(self, idx):
        data = torch.load(os.path.join(self.processed_dir, "data_nmas{}.pt".format(idx)))
        return data
...
##-----------------------nihang-modification-----------------------------
##Process masking and add atomic stiffness
def process_data(data_path, processed_path, processing_args):
    ...
    ##Load dictionary
    print("test: ", processing_args["dictionary_source"])
    ...
    # print("shape pf atom_dictionary£º ", atom_dictionary)
    ...
    for index in range(0, len(target_data)):
        ...
        # print("AAAA: ", len(data.y), [[structure_id]])
    ...
    if processing_args["dictionary_source"] != "generated":
        ...
        for index in range(0, len(data_list)):
            # print("atom_dict: ", np.shape(atom_dictionary[str(data_list[index].ase.get_atomic_numbers()[0])]))
            atom_fea = np.vstack(
                [
                    atom_dictionary[str(data_list[index].ase.get_atomic_numbers()[i])]
                    for i in range(len(data_list[index].ase))  # number of atom
                ]
            ).astype(float)

            data_list[index].x = torch.Tensor(atom_fea)
            #print("atom_dict: ", np.shape(data_list[index].x))  ## [x, 100] atom feature
    ...
    for index in range(0, len(data_list)):
        ...
        # print("atom_dict: ", np.shape(data_list[index].x))   [x,114]  14--> node degree
    ...
    data_list = DataMasking(data_list)
    ...
    if processing_args["dataset_type"] == "inmemory":
        ...
        torch.save((data, slices), os.path.join(data_path, processed_path, "data_nmas.pt"))
        ...
        for i in range(0, len(data_list)):
            torch.save(
                data_list[i],
                os.path.join(
                    os.path.join(data_path, processed_path), "data_nmas{}.pt".format(i)
                ),
            )
...
################################################################################
as_path = os.path.join(os.path.abspath(os.curdir), 'matdeeplearn/process/atomic_stiffness_default.npy')
as_dict = np.load(as_path, allow_pickle=True).item()

##data mask + AS. 
##One atom only mask once. Mask ratio 0.15 
def DataMasking(data_list):
    
    for index in range(len(data_list)):
        # data = data_list[index]        
        zero_col = torch.zeros(data_list[index].x.shape[0],1)   # [x, 1]
        data_list[index].x = torch.cat([data_list[index].x, zero_col.to(data_list[index].x.dtype)], dim=-1)  # [x, 114 +1] --> [x, 115]        
        data = data_list[index]

        num_atoms = data.x.size()[0]
        sample_size = int(num_atoms * 0.15 + 1)   # 0.15 is mask ratio
        masked_atom_indices = random.sample(range(num_atoms), sample_size)
        # print("atom index: ", masked_atom_indices)

        # create mask node label by copying atom feature of mask atom
        mask_node_labels = torch.zeros(len(masked_atom_indices), len(data.x[0]))
        # print("shape of masked_atom_indices: ", mask_node_labels.shape)
        
        tmp = []
        count = 0
        for atom_idx in masked_atom_indices:
            one_idx = torch.nonzero(data.x[atom_idx][:100]==1).squeeze()
           
            #print(as_dict, one_idx)
            tmp.append(as_dict[int(one_idx)+1])
            
            
            mask_node_labels[count, :] = data.x[atom_idx]
            data.x[atom_idx][: -15] = 0
            data.x[atom_idx][-1] = 1 
            count += 1
            
        data.y = mask_node_labels
        data.y1 = torch.Tensor(tmp)
        
        #print(data.y.shape, data.y1.shape)
        
        data.atom_index = torch.tensor(masked_atom_indices)
        # print("shape of x and y and index: ", data.x, data.y, len(data.atom_index))
...
def OneHotDegree(data, max_degree, in_degree=False, cat=True):   # Here set max_degree = 13
    idx, x = data.edge_index[1 if in_degree else 0], data.x
    # print("test idx and x: ", idx, x)    
    ## why?
    deg = degree(idx, data.num_nodes, dtype=torch.long)
    #print("test deg1: ", deg, np.shape(deg))
    deg = F.one_hot(deg, num_classes=max_degree + 1).to(torch.float)
    #print("test deg2: ", deg, np.shape(deg))

    if x is not None and cat:
        x = x.view(-1, 1) if x.dim() == 1 else x
        data.x = torch.cat([x, deg.to(x.dtype)], dim=-1)  # 100 + 14 --> [x, 114]
    else:
        data.x = deg
