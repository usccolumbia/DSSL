...
def get_dataset(save_data, data_path, target_index, reprocess="False", processing_args=None):
    ...
    if reprocess == "True":
        ...
        process_data(save_data, data_path, processed_path, processing_args)

    if os.path.exists(os.path.join(data_path, processed_path, save_data + '.pt')) == True:
    ...
    elif os.path.exists(os.path.join(data_path, processed_path, save_data + '0.pt')) == True:
...
class StructureDataset(InMemoryDataset):
    def __init__(
        self, save_data, data_path, processed_path="processed", transform=None, pre_transform=None
    ):
...
class StructureDataset_large(Dataset):
    ...
    def processed_file_names(self):
        # file_names = ["data.pt"]
        file_names = []
        for file_name in glob.glob(self.processed_dir + "/data_cl*.pt"):
    ...
    def get(self, idx):
        data = torch.load(os.path.join(self.processed_dir, "data_cl{}.pt".format(idx)))
        return data
...
def process_data(save_data, data_path, processed_path, processing_args):
    ...
    # print("shape pf atom_dictionary ", atom_dictionary)
    ...
    for index in range(0, len(target_data)):
        ...
        struct = AseAtomsAdaptor.get_structure(ase_crystal)
        struct.perturb(0.5, min_distance=0.1)  # distance = 0.5, min_distance = 0.1
        ase_crystal = AseAtomsAdaptor.get_atoms(struct)
    ...
    if processing_args["dataset_type"] == "inmemory":
        ...
        torch.save((data, slices), os.path.join(data_path, processed_path, save_data+'.pt'))
        ...
        for i in range(0, len(data_list)):
            torch.save(
                data_list[i],
                os.path.join(
                    os.path.join(data_path, processed_path), save_data+"{}.pt".format(i)
                ),
            )
...
def OneHotDegree(data, max_degree, in_degree=False, cat=True):   # Here set max_degree = 13
    ...
    if x is not None and cat:
        ...
        zero_col = torch.zeros(x.shape[0],1)   # [x, 1]
        data.x = torch.cat([x, deg.to(x.dtype), zero_col.to(x.dtype)], dim=-1)  # 100 + 15 --> [x, 115]
    else:
        data.x = deg
