...
def get_dataset(data_path, target_index, reprocess="False", processing_args=None):
    ...
    if os.path.exists(os.path.join(data_path, processed_path, "data_nmve.pt")) == True:
        dataset = StructureDataset(
            data_path,
            processed_path,
            transforms,
        )
    elif os.path.exists(os.path.join(data_path, processed_path, "data_nmve0.pt")) == True:
...
class StructureDataset_large(Dataset):
    ...
    def processed_file_names(self):
        # file_names = ["data.pt"]
        file_names = []
        for file_name in glob.glob(self.processed_dir + "/data_nmve*.pt"):
    ...
    def get(self, idx):
        data = torch.load(os.path.join(self.processed_dir, "data_nmve{}.pt".format(idx)))
        return data
...
##-----------------------nihang-modification-----------------------------
##Process masking and add valence electron
def process_data(data_path, processed_path, processing_args):
    ...
    if processing_args["dataset_type"] == "inmemory":
        ...
        torch.save((data, slices), os.path.join(data_path, processed_path, "data_nmve.pt"))
        ...
        for i in range(0, len(data_list)):
            torch.save(
                data_list[i],
                os.path.join(
                    os.path.join(data_path, processed_path), "data_nmve{}.pt".format(i)
                ),
            )
...
################################################################################
ve_path = os.path.join(os.path.abspath(os.curdir), 'matdeeplearn/process/ve_dict_default.npy')
ve_dict = np.load(ve_path, allow_pickle=True).item()
...
def DataMasking(data_list):
    ...
    for index in range(len(data_list)):
        ...
        for atom_idx in masked_atom_indices:
            ...
            #print(ve_dict, one_idx)
            tmp.append(ve_dict[int(one_idx)+1])
